...
"""
데코레이터: 뷰함수가 실행되기전에 먼저 실행되는 함수
login_required : 뷰함수가 실행되기전 웹클라이언트의 로그인 여부를 파악하고
비로그인 상태인 웹글라이언트에게 로그인페이지를 리다이렉트 해주는 데코레이터
...
def detail(request,q_id):
    ...
    #또는 조건에 맞는 객체가 없으면 404번에러를 클라이언트에 전달
    q = get_object_or_404(Question, id = q_id )
    '''
    get_object_or_404(모델클래스,조건):모댈클래스에 조건을 검색해 1개의 객체 호출
    만약 객체가 없는 경우, 클라이언트의 잘못된 접근으로 판단해 뷰함수를 종료하고 404에러 전달
    
    현재 Qusetion 모댈클래스는 choice 모델클래스가 외래키로 연결한 상태이므로,
    question 객체들은 자신과 연결된 choice 객체들을 추출 할 수있음
...
def vote(request):
    ...
    if request.method == "POST":
        '''
        request.POST또는 request.GET:웹클라이언트의 요청과 함께 날라온 데이터를 저장한 변수
        값을 꺼낼때, HTML코드에 name 속성 이름으로 값을 추출할 수 있다
        '''
        print(request.POST)
        ...
        #c.q:choice 객체가 연결한 question 객체 변수
        #c.q.id: 연결한 question 객체의 id변수값
...
from _datetime import datetime#컴퓨터의 현재 날짜/시간을 대입
#데이터베이스에 새로만든 모델겍체 저장
#모델객체.save() : 새로운 객체를 저장하거나 기존 객체의 변수값변경을 데이터베이스에 저장할 수있다
#Question 객체 추가
@login_required
...
def qupdate(request,q_id):
    ...
    #POST 방식 요청
        ...
        #수정을 하는 객체를 바탕으로 QuestionForm 객체가 생성됬기떄문에
    #>바로 데이터베이스에 저장
    w = obj.save()
    print('수정할 객체 q의 id:',q_id)
